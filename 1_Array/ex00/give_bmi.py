...
def give_bmi(height: list[int | float], weight: list[int | float]) -> list[int | float]:
  """
  1/ verifier que height et weight ont la meme taille et que ce sont des int/float
  ...
  if not all(isinstance(x, (int, float)) for x in height):
     print("AssertionError: elements in height list must be float or int only.")
     return

  if not all(isinstance(x, (float, int)) for x in weight):
     print("AssertionError: elements in wieght list must be float or int only.")
     return
  
  pairs = zip(height, weight)
  # print(list(pairs))
  newList = []
